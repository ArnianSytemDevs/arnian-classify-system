...
export class entryFormController {
    ...
    public static async getClient(filter: string | undefined,mode: "edit" | "create", isPrefilled = false) {
    try {
        let list;
        const hasValue = typeof filter === "string" && filter.trim() !== "";

        if (mode === "edit" && isPrefilled && hasValue) {
        list = await getClientsList(undefined, 10, {id: filter,});
        } else if (hasValue) {
        list = await getClientsList(undefined, 10, { name: filter,});
        } else {
    ...
    public static async createEntry( entryForm: EntryForm, status: Status[], mode: string, entrySel?: Entry ) {
        try {
